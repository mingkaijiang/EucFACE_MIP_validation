calculate_normalized_delta_vegetation_pool_response <- function(inDF, pcycle) {
    ...
    if (pcycle == F) {
        ...
        for (i in trt) {
            for (j in modlist) {
                tmpDF$CL[tmpDF$Trt==i&tmpDF$ModName==j] <- mean(inDF$deltaCL[inDF$Trt==i&inDF$ModName==j], na.rm=T)
