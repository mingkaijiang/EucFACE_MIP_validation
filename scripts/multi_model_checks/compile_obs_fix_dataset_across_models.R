compile_obs_fix_dataset_across_models <- function(p.mod.list, n.mod.list, d.mod.list) {
    ...
    for (mod.abb in p.mod.list) {
        ...
        if (mod.abb=="CABLP") {
            modDF <- read.csv(paste0("simulation_output/", mod.abb, 
                                     "/forest/EUC_", mod.abb, "_OBS_FIX_AMB_NOP_D.csv"))
    ...
    for (mod.abb in p.mod.list) {
        ...
        if (mod.abb=="CABLP") {
            modDF <- read.csv(paste0("simulation_output/", mod.abb, 
                                     "/forest/EUC_", mod.abb, "_OBS_FIX_ELE_NOP_D.csv"))
        } else if (mod.abb=="LPJGP") {
            modDF <- read.csv(paste0("simulation_output/", mod.abb, 
    ...
    ambDF <- readRDS(paste0(out.dir, "/MIP_obs_fix_amb_daily.rds"))
    eleDF <- readRDS(paste0(out.dir, "/MIP_obs_fix_ele_daily.rds"))
    ...
    ### save output
    saveRDS(annDF1, paste0(out.dir, "/MIP_obs_fix_amb_annual.rds"))
    saveRDS(annDF2, paste0(out.dir, "/MIP_obs_fix_ele_annual.rds"))
