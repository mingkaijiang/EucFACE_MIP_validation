EucFACE_mass_balance_and_validation_script_OCHDX <- function() {
    ...
    modDF <- read.csv(paste0("simulation_output/OCHDX/EUC_", mod.abb, "_OBS_VAR_AMB_NOP_D.csv"))
    ...
    print(modDF$DOY[modDF$YEAR==2012])
    ...
    # DSG: TRANS, DEP, PWEA don't exist not in output protocol
    ...
    # DSG: tissue respiration not reported
    ...
    # DSG: CLITIN (only C into litter pool / respired fraction missing)
    p6<-plot.new()
    ...
    # DSG: missing litter fluxes from CSTOR & REPR ORGANS
    ...
    #p14<-xyplot(I(CEX+CLITIN+CWLIN+CFRLIN+CCRLIN-RHET)~I(deltaCSOIL+deltaCCLITB+deltaCFLIT),annDF,
    #           auto.key=T,
    #           scales=list(relation='free'),
    #           panel=function(...){
    #               panel.xyplot(...)
    ...
    p6<-plot.new()
    #DSG: no leave/froot retronsl reported
    ...
    # DSG: NSTOR WAS MISSING:
    p10<-xyplot(I(NDEP+NFIX-NLEACH-NVOL)~(I(deltaNSTOR+deltaNL+deltaNW+deltaNCR+deltaNFR+deltaNSOIL+deltaNFLIT+deltaNCLITB)),annDF,
    ...
    #DSG: litter inputs from reserves are not reported:
    ...
    #DSG:  P losses to strong soprtion not reported
    p1<-plot.new()
    ...
    #DSG: str. sorptio loses not reported
    p10<-plot.new()
    ...
    #DSG: PSEC+POCC+PPAR not simualted, PPMIN not defined
    p11<-plot.new()
    ...
    #DSG: PSEC+POCC+PPAR not simualted
    p12<-plot.new()
