read_and_process_EucFACE_P_budget_output_not_ignore_understorey <- function() {
    ...
    ## deltaPL
    outDF$deltaPL[outDF$Group=="mean"&outDF$Trt=="aCO2"] <- deltaDF$aCO2[deltaDF$terms=="Canopy P Pool"]+deltaDF$aCO2[deltaDF$terms=="Understorey P Pool"]
    outDF$deltaPL[outDF$Group=="mean"&outDF$Trt=="eCO2"] <- deltaDF$eCO2[deltaDF$terms=="Canopy P Pool"]+deltaDF$eCO2[deltaDF$terms=="Understorey P Pool"]
    outDF$deltaPL[outDF$Group=="sd"&outDF$Trt=="aCO2"] <- sqrt((deltaDF$aCO2_sd[deltaDF$terms=="Canopy P Pool"]^2+deltaDF$aCO2_sd[deltaDF$terms=="Understorey P Pool"]^2)/2)*2
    outDF$deltaPL[outDF$Group=="sd"&outDF$Trt=="eCO2"] <- sqrt((deltaDF$eCO2_sd[deltaDF$terms=="Canopy P Pool"]^2+deltaDF$eCO2_sd[deltaDF$terms=="Understorey P Pool"]^2)/2)*2
    ...
    ## deltaCFR
    tmp <- colSums(deltaDF[deltaDF$terms%in%c("Fine Root P Pool"),2:7], na.rm=T)
    amean <- mean(c(tmp[2], tmp[3], tmp[6]), na.rm=T)
    emean <- mean(c(tmp[1], tmp[4], tmp[5]), na.rm=T)
    asd <- sd(c(tmp[2], tmp[3], tmp[6]), na.rm=T)
    esd <- sd(c(tmp[1], tmp[4], tmp[5]), na.rm=T)
