convert_into_annual_CABLP <- function (inDF) {
    ...
    fluxDF <- summaryBy(PREC+ET+TRANS+ES+EC+RO+DRAIN+NEP+GPP+NPP+RHET+RAU+RECO+CGL+CGFR+CGCR+CGW+NGL+NGFR+NGCR+NGW+PGL+PGFR+PGCR+PGW+
                            ...
                            NDEP+NFIX+NVOL+PDEP+PWEA+CSTORFLX+NGMINCWD+PGMINCWD~YEAR, data=inDF, FUN=sum, keep.names=T, na.rm=T)
    
    
    ### subset first day within a year of all pools
    poolDF <- inDF[,c("YEAR", "DOY", "SW","SWHC","CL","LAI","CW","CFR","CCR","NL","NW","NFR","NCR","PL","PW","PFR","PCR","CSTOR","NSTOR","PSTOR",
