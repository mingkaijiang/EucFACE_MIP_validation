read_and_process_EucFACE_C_budget_output_not_ignore_understorey <- function() {
    ...
    outDF$CL[outDF$Group=="mean"&outDF$Trt=="eCO2"] <- poolDF$eCO2[poolDF$term=="Overstorey leaf"]+poolDF$eCO2[poolDF$term=="Understorey above-ground"]
    outDF$CL[outDF$Group=="sd"&outDF$Trt=="aCO2"] <- sqrt((poolDF$aCO2_sd[poolDF$term=="Overstorey leaf"]^2+poolDF$aCO2_sd[poolDF$term=="Understorey above-ground"]^2)/2)
    outDF$CL[outDF$Group=="sd"&outDF$Trt=="eCO2"] <- sqrt((poolDF$eCO2_sd[poolDF$term=="Overstorey leaf"]^2+poolDF$eCO2_sd[poolDF$term=="Understorey above-ground"]^2)/2)
    outDF$CL[outDF$Group=="mean"&outDF$Trt=="diff"] <- outDF$CL[outDF$Group=="mean"&outDF$Trt=="eCO2"] - outDF$CL[outDF$Group=="mean"&outDF$Trt=="aCO2"]
    outDF$CL[outDF$Group=="sd"&outDF$Trt=="diff"] <- sqrt((outDF$CL[outDF$Group=="sd"&outDF$Trt=="aCO2"]^2+
    ...
    ### CGL
    tmp <- colSums(nppDF[nppDF$term%in%c("Leaf NPP", "Leaf consumption", "Understorey NPP"),2:7], na.rm=T)
    amean <- mean(c(tmp[2], tmp[3], tmp[6]), na.rm=T)
    emean <- mean(c(tmp[1], tmp[4], tmp[5]), na.rm=T)
    asd <- sd(c(tmp[2], tmp[3], tmp[6]), na.rm=T)
    esd <- sd(c(tmp[1], tmp[4], tmp[5]), na.rm=T)
    ...
    tmpDF <- nppDF[nppDF$term%in%c("Leaf NPP", "Stem NPP", "Understorey NPP",
                                   "Fine Root NPP", "Intermediate Root NPP",
                                   "Coarse Root NPP", "Other NPP",
                                   "Leaf consumption", "Mycorrhizal production"),1:7]
    #tmpDF[tmpDF$term=="Fine Root NPP",2:7] <- tmpDF[tmpDF$term=="Fine Root NPP",2:7] 
    #tmpDF[tmpDF$term=="Intermediate Root NPP",2:7] <- tmpDF[tmpDF$term=="Intermediate Root NPP",2:7]
    
    tmp <- colSums(tmpDF[tmpDF$term%in%c("Leaf NPP", "Stem NPP", "Understorey NPP",
                                         "Fine Root NPP", "Intermediate Root NPP",
                                         "Coarse Root NPP", "Other NPP",
    ...
    ### RL
    outDF$RL[outDF$Group=="mean"&outDF$Trt=="aCO2"] <- inoutDF$aCO2[inoutDF$term=="Ra leaf"]+inoutDF$aCO2[inoutDF$term=="Ra understorey"]
    outDF$RL[outDF$Group=="mean"&outDF$Trt=="eCO2"] <- inoutDF$eCO2[inoutDF$term=="Ra leaf"]+inoutDF$eCO2[inoutDF$term=="Ra understorey"]
    outDF$RL[outDF$Group=="sd"&outDF$Trt=="aCO2"] <- sqrt((inoutDF$aCO2_sd[inoutDF$term=="Ra leaf"]^2+inoutDF$aCO2_sd[inoutDF$term=="Ra understorey"]^2)/2)
    outDF$RL[outDF$Group=="sd"&outDF$Trt=="eCO2"] <- sqrt((inoutDF$eCO2_sd[inoutDF$term=="Ra leaf"]^2+inoutDF$eCO2_sd[inoutDF$term=="Ra understorey"]^2)/2)
    outDF$RL[outDF$Group=="mean"&outDF$Trt=="diff"] <- outDF$RL[outDF$Group=="mean"&outDF$Trt=="eCO2"] - outDF$RL[outDF$Group=="mean"&outDF$Trt=="aCO2"]
    ...
    tmp <- colSums(inoutDF[inoutDF$term%in%c("Ra leaf", "Ra stem", "Ra understorey",
                                         "Ra root", "Rgrowth"),2:7], na.rm=T)
    amean <- mean(c(tmp[2], tmp[3], tmp[6]), na.rm=T)
    emean <- mean(c(tmp[1], tmp[4], tmp[5]), na.rm=T)
    asd <- sd(c(tmp[2], tmp[3], tmp[6]), na.rm=T)
    esd <- sd(c(tmp[1], tmp[4], tmp[5]), na.rm=T)
    ...
    ### RECO
    tmp <- colSums(rbind(inoutDF[inoutDF$term%in%c("Ra leaf", "Ra understorey", "Ra stem", "Ra root",
                                                   "Rgrowth"),2:7], 
