EucFACE_mass_balance_and_validation_script_CABLP <- function(forest) {
    ...
    fluxDF <- summaryBy(PREC+ET+TRANS+ES+EC+RO+DRAIN+NEP+GPP+NPP+RHET+RAU+RECO+CGL+CGFR+CGCR+CGW+NGL+NGFR+NGCR+NGW+PGL+PGFR+PGCR+PGW+
                        ...
                        CCRLIN+CFRLIN+CWLIN+NLITIN+NCRLIN+NFRLIN+NWLIN+PLITIN+PCRLIN+PFRLIN+PWLIN+NWRETR+PWRETR+NCRRETR+PCRRETR+NFRRETR+PFRRETR+
                        NDEP+NFIX+NVOL+PDEP+PWEA+CSTORFLX+NGMINCWD+PGMINCWD~YEAR, data=modDF, FUN=sum, keep.names=T, na.rm=T)
    
  
    ### subset first day within a year of all pools
    poolDF <- modDF[,c("YEAR", "DOY", "SW","SWHC","CL","LAI","CW","CFR","CCR","NL","NW","NFR","NCR","PL","PW","PFR","PCR","CSTOR","NSTOR","PSTOR",
    ...
    p14<-xyplot(I(CLITIN+CWLIN+CFRLIN-RHET)~I(deltaCSOIL+deltaCFLIT+deltaCCLITB),annDF,
                auto.key=T,
                scales=list(relation='free'),
                panel=function(...){
                    panel.xyplot(...)
    ...
    pdf(paste0(out.dir, '/QC_Carbon_Balance_',mod.abb,'_',version,'.pdf',sep=''),width=10,height=8)
    for (i in 1:14) {
    ...
    p2<-xyplot(I(PREC-(RO+DRAIN+ET))~deltaSWHC, annDF,
    ...
    ### print plots to file, change numbering if needed
    pdf(paste0(out.dir, '/QC_Water_Balance_',mod.abb,'_',version,'.pdf',sep=''),width=10,height=8)
    for (i in 1:2) {
        print(get(paste("p",i,sep="")))
    }
    dev.off()
    ...
    p2<-xyplot(I(NGW-NWLIN)~deltaNW,annDF,
    ...
    p3<-xyplot(I(NGFR-NFRLIN)~deltaNFR,annDF,
    ...
    p4<-xyplot(I(NGCR-NCRLIN)~deltaNCR,annDF,
    ...
    p6<-xyplot(I(NUP)~I(NGL+NGFR+NGW),annDF,
    ...
    p7<-xyplot(I(NUP-NGL-NGFR-NGW)~deltaNSTOR,annDF,
    ...
    p8<-xyplot(I(NFIX+NUP)~I(NGL+NGFR+NGW),annDF,
    ...
    p9<-xyplot(I(NFIX+NUP-NGL-NGFR-NGW)~deltaNSTOR,annDF,
    ...
    ## JK: NGMINCWD (N CWD gross mineralisation) added to close mass balance
    p11<-xyplot(I(NDEP+NFIX+NLITIN+NFRLIN-NUP-NLEACH-NVOL+NGMINCWD)~I(deltaNSOIL+deltaNFLIT),annDF,
    ...
    ### print plots to file, change numbering if needed
    pdf(paste0(out.dir, '/QC_Nitrogen_Balance_',mod.abb,'_',version,'.pdf',sep=''),width=10,height=8)
    for (i in 1:13) {
        print(get(paste("p",i,sep="")))
    }
    dev.off()
    ...
    p1<-xyplot(I(PDEP+PWEA-PLEACH)~(I(deltaPL+deltaPW+deltaPFR+deltaPSOIL+deltaPFLIT+deltaPCLITB)),annDF,
    ...
    p2<-xyplot(I(PGL-PLITIN)~deltaPL,annDF,
    ...
    p3<-xyplot(I(PGW-PWLIN)~deltaPW,annDF,
    ...
    p4<-xyplot(I(PGFR-PFRLIN)~deltaPFR,annDF,
    ...
    p6<-xyplot(I(PGCR-PCRLIN)~deltaPCR,annDF,
    ...
    p8<-xyplot(I(PUP)~I(PGL+PGFR+PGW),annDF,
    ...
    p9<-xyplot(I(PUP-PGL-PGFR-PGW)~deltaPSTOR,annDF,
    ...
    ## JK: PGMINCWD (P CWD gross mineralisation) added to close mass balance
    p10<-xyplot(I(PDEP+PWEA+PLITIN+PFRLIN-PUP-PLEACH+PGMINCWD)~I(deltaPSOIL+deltaPFLIT),annDF,
    ...
    p13<-xyplot(I(PLAB+PSEC+POCC+PPORG)~PSOIL,annDF,   # no parent material in CABLE
    ...
    ### print plots to file, change numbering if needed
    pdf(paste0(out.dir, '/QC_Phosphorus_Balance_',mod.abb,'_',version,'.pdf',sep=''),width=10,height=8)
    for (i in 1:13) {
        print(get(paste("p",i,sep="")))
    }
    dev.off()
    ...
    ### print plots to file, change numbering if needed
    pdf(paste0(out.dir, '/QC_Time_averaged_validation_',mod.abb,'_',version,'.pdf',sep=''),width=10,height=8)
    for (i in 1:9) {
        print(get(paste("p",i,sep="")))
    }
    dev.off()
    ...
    subDF <- modDF[,c("YEAR", "DOY", "Date", "SWHC")]
    ...
    p4 <- ggplot(plotDF, aes(x=Date)) +
        ...
        geom_point(aes(y=SWHC, color="sim"), lwd = 1) +
        theme_linedraw() +
        theme(panel.grid.minor=element_blank(),
              axis.title.x = element_text(size=14), 
              axis.text.x = element_text(size=12),
    ...
    ### print plots to file, change numbering if needed
    pdf(paste0(out.dir, '/QC_Time_varying_validation_',mod.abb,'_',version,'.pdf',sep=''),width=10,height=8)
    for (i in 1:4) {
        print(get(paste("p",i,sep="")))
    }
    dev.off()
