...
fluxDF <- summaryBy(ET+Transpiration+ES+EC+RO+DRAIN+NEP+GPP+NPP+RHET+RAU+CGL+CGFR+CGCR+CGW+NGL+NGFR+NGCR+NGW+PGL+PGFR+PGCR+PGW+NUP+NGMIN+NMIN+NLEACH+PUP+PGMIN+PMIN+PLEACH+PBIOCHMIN+NLRETR+PLRETR+RCR+RFR+RCR+RFR+CREPR+CEX+CVOC+RL+RW+RGR~year, data=modDF, FUN=sum, keep.names=T, na.rm=T)
...
poolDF <- modDF[,c("year", "doy", "CL","LAI","CW","CFR","CCR","NL","NW","NFR","NCR","PL","PW","PFR","PCR","CSTOR","NSTOR","PSTOR",
                   ...
                   "NFLITA","NFLITB","PFLITA","PFLITB","CCLITB","NCLITB","PCLITB")]
...
for (i in c(2012:2018)) {
    deltaDF[deltaDF$year==i,3:l] <- poolDF[poolDF$year==(i+1),3:l]-poolDF[poolDF$year==i,3:l]
}
