...
########################################################################################################
### revise the model output names following output protocol 
### Note that the following lines of code in this chunk only apply to GDAY output
### other models may need to modify this section as they see appropriate
...
########################################################################################################
### add extra column where GDAY simulation doesn't output 
### Note that the following lines of code in this chunk only apply to GDAY output
### other models may need to modify this section as they see appropriate
...
Ecosystem respiration (RECO) should be the sum of autotrophic and heterotrophic respiration. 
...
This is to sum all autotrophic respiration fluxes. Note that some models may not explicitly similate individual respiratory fluxes. Hence, this mass balance check may not be closed for some models. 
...
This is to check the mass balance on change in leaf carbon pool. $\Delta$CL should equals influx - outflux. Same expectation for $\Delta$CW, $\Delta$CFR, and $\Delta$CCR.
...
GPP should in theory equals to production flux and respiratory flux. Here we include reproduction flux as well, which some models don't simulate. 
...
This is a different way to check mass balance for NPP, in that in includes $\Delta$CSTOR. Some models don't explictly simulate CSTOR, so this mass balance may not apply. 
...
The total fine litterfall flux should be the sum of aboveground and belowground fluxes. 
...
This mass balance equation checks the net of total influx litter and heterotrophic respiration. In theory, the net difference of these two flues should equal to the change in soil + litter pool. 
...
The water budget will need to be closed in order to investigate the CO2 x water interaction. Firstly, the total evapotranspiration flux should equal to total transpiration + evaporation flux. More checks to be added. 
...
Now we plot some nitrogen balance checks. Firstly, we check $\Delta$NL, $\Delta$NW, $\Delta$NFR and $\Delta$NCR. Then we will check litterfall fluxes and changes in soil pools.  
...
Finally, we plot the phosphorus balance checks. 
