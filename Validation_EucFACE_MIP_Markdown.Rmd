...
p1 <- ggplot(testDF1, aes(x=Date)) +
        geom_errorbar(aes(ymin=lai.y-laiSD,
                        ymax=lai.y+laiSD, color="lai.y"))+
        geom_line(aes(y=lai.x, color="lai.x"), lwd = 1) +
        theme_linedraw() +
        ...
        ylab("LAI")+
        scale_colour_manual("", 
                            values = c("lai.x"="red2", "lai.y"="black"),
...
lm.fit <- lm(testDF2$lai.y~testDF2$lai.x)


### plot all data
p1 <- ggplot(testDF2, aes(x=lai.y, y=lai.x)) +
        geom_point()+
        theme_linedraw() +
